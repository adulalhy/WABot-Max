...
exports.convertSticker = (base64, author, pack) => {
...
async function check(buffer) {
    let found = await stickers.find(v => v.mediaKey == buffer) || false
    return found
  }
  
exports.addStickCmd = (mediaKey, comando) => {
  stickers.push({mediaKey: mediaKey, cmd: comando})
    fs.writeFileSync('./database/json/stickCmd.json', JSON.stringify(stickers, null, 4))
}

exports.getStick = async(base64) => {
  const verify = await check(base64)
  return verify ? `_Esta sticker já está associada com o comando *${verify.cmd}*_` : false
}